...
def main():
    ...
    #Health Check - macOS
    pending_config_results = (evaluate_configs(mi_device_configs))
    export_results(pending_config_results)
...
def evaluate_configs(mi_device_configs):
        
    print('---')
    print("Evaluating Configurations")

    pending_config_results = []

    bar_configs = Bar('Evaluating...', suffix='%(index)d')
    ...
    for i in mi_device_configs:
        ...
        # Filter out functioning config statuses
            if str.upper(i2['status']) not in ('INSTALLED', 'ACTIVE', 'UNINSTALLED'):
                ...
                stuck_configs.append({'name':i2['name'], 'status':i2['status']})
        # Add device info to new object and append stuck configs
        if len(stuck_configs) > 0:
            pending_config_results.append(i)
            pending_config_results[pending_config_results.index(i)].update({'stuck_configs':stuck_configs})
            pending_config_results[pending_config_results.index(i)].update({'total_stuck_configs':len(stuck_configs)})
        bar_configs.next()
    bar_configs.finish()

    if len(pending_config_results) > 0:
        print (f"Found {len(pending_config_results)} devices with pending or error configurations.")

    return pending_config_results

def export_results(pending_config_results):
    ...
    if len(pending_config_results) > 0:
        with open(SCRIPT_RESULTS / f'macOS_pending_config_results_{TODAY}.csv', 'w', newline='') as outFile:
            writer = csv.DictWriter(outFile, fieldnames=csv_columns)
            writer.writeheader()
            for i in pending_config_results:
